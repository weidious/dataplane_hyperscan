...
void scan_dns_ring(__rte_unused void *args)
{
	...
	//		(struct epc_meta_data *)RTE_MBUF_METADATA_UINT8_PTR(pkts, META_DATA_OFFSET);
	//	if (meta_data->dns) {
	//		printf("hi\n");
	...
	//	if(dns_payload_off==data_payload_off)
	//	printf("Size=%d",rte_pktmbuf_data_len(pkts));
	//	offline_scan(rte_pktmbuf_mtod(pkts, char *) + dns_payload_off,rte_pktmbuf_data_len(pkts) - dns_payload_off);
	//char* tmp = rte_pktmbuf_mtod(pkts, char *) + dns_payload_off;
		//unsigned long patt = tmp[0]& 128 ;
		//printf("Test first patload char : %lu",patt);
	//	if(tmp[0]=='A')
	//		printf("Test first patload char : %c",tmp[0]);
